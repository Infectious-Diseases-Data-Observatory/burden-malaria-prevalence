...
**Implementation transition.** The seven existing single-imputation fits are available as `age_1.rds` through `age_7.rds` under `data/derived_cbh/models/age_band_hiv_incidence_shared_time_v3/sensitivity_single_imputation/`, fitted by `R_cbh/sensitivity/01_fit.R`. They implement the selected primary structure. Promoting them to named primary outputs, propagating HIV uncertainty through the separate fits, refitting geographic/period subsets using that structure, and updating downstream primary curves and burden estimates are subsequent implementation steps. Existing joint-model results, the key comparison figure and country burden estimates retain their original provenance; this plan update does not relabel or recompute them.
