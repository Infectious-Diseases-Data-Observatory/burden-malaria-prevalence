...
**Current fitted version:** the [regional primary manifest](../results/cbh/primary_map_regional18_gamma2_v2/fit_manifest.csv) records seven new MAP gamma=2 fits using **5,680,117 child-band records, 1,755,838 children and 78,634 deaths in 973 survey-regions, 100 surveys and 34 countries**. All seven passed convergence, rank, finite-covariance, positive smoothing-Hessian, exact fitted-input and compact-PfPR prediction checks. The [iteration comparison](../results/cbh/primary_map_regional18_gamma2_v2/REPORT.md) includes spline overlays, contrasts, sample accounting and national estimates for 2005/2015/2024.

**Previously fitted benchmark (superseded adjustment set):** preserve [primary_map_gamma2_v1](../results/cbh/primary_map_gamma2_v1/fit_manifest.csv), which uses the previous eleven-variable individual/country adjustment. Existing paper figures, 2004–2024 annual comparisons, Nigerian state estimates and supplementary fits remain from that earlier iteration until separately regenerated. Do not relabel those artifacts as revised regional-adjustment results. `Rscript run_all.R --primary` now runs the revised version; see [current pipeline instructions](../R_cbh/primary/REGIONAL_REFIT.md).
...
**Figure-version status:** the paper artifacts linked below remain from `primary_map_gamma2_v1`; the current revised curve and comparison plots are in `primary_map_regional18_gamma2_v2`. Regenerate manuscript artifacts and their captions from the revised version before treating them as current results. This refit did not write manuscript or TeX files.
...
**17 September regional refit:** [run_regional.R](../R_cbh/primary/run_regional.R) now implements the current 18-variable primary version, including preparation from existing audited inputs, seven fresh fits, effects, 2005/2015/2024 national burden, aggregate diagnostics and iteration comparisons. The new outputs and previous benchmark are kept separate.
...
`Rscript run_all.R --primary` forces fresh revised primary fits after verifying/assembling the saved regional input overlay. `Rscript R_cbh/primary/run_regional.R --resume` reuses only valid fit caches; `--report-only` uses saved fits for effects, diagnostics and comparisons. The new runner performs no raw-source extraction, HIV refit, supplementary fitting, TeX generation or Overleaf edits. The older `primary/run.R` is retained for historical reporting and includes a TeX writer; do not invoke it for the current primary version or while the standing no-TeX-edit instruction applies. `--audit` verifies the preserved older snapshot; `--check-inputs` checks local source prerequisites.
...
1. Regenerate the remaining annual/state burden, sample flow, manuscript figures and sensitivities from the verified regional fits, preserving previous-version outputs and respecting the no-TeX-edit instruction. The seven revised primary fits, 2005/2015/2024 national estimates and iteration comparison are complete. Resolve the remaining regional-source gaps and assess selection/imputation sensitivity. In-sample fitted-outcome checks supplement numerical diagnostics but are not cross-validation or influence analysis.
