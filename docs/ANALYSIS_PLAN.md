...
Updated 17 September 2026 (expanded regional adjustment and national UNICEF vaccination fallback). This is a working analysis plan revised after inspection of exploratory results, not a prospective preregistration. The [previous plan](ANALYSIS_PLAN_BEFORE_MAP_GAMMA2.md) preserves the original audit, superseded specifications and decision history; the [code audit](CODE_AUDIT.md) compares each component with the current implementation and records the archival changes.

**Current primary analysis:** seven separate child–age-band mortality models using **annual MAP PfPR₂–₁₀ at band entry**, fitted with **`mgcv::bam`, `gamma = 2`, PfPR `bs = "cr", k = 5`, and calendar-year `bs = "cr", k = 6`**. Use the full eligible MAP sample, including eligible observations after 2015. Each age band has its own time spline, confounder coefficients and survey/country/region random effects and variance parameters. Section 3.1 gives the complete specification. **The 17 September revision replaces individual confounders with survey-region averages and adds five vaccine measures, facility delivery, exclusive breastfeeding, short birth interval, WASH and electricity. Missing regional vaccination coverage is now replaced by exact country/survey-year UNICEF WUENIC estimates where available, with source flags. This revised specification has not yet been fitted.**
...
Survey-region DHS coverage, wealth, urban residence and education describe survey-time conditions or the indicator's stated retrospective window. Do not relabel them as reconstructed historical values at band entry. The five vaccine measures are all required after the national UNICEF fallback described below. Preserve source-missing, non-introduction and no-series flags; an assumed pre-series/no-series zero is not an observed coverage estimate. Maternal death fraction, hypothetical children and wasting remain outside the adjustment set.

**National UNICEF vaccination imputation:** use a deterministic country-level substitution, not a fitted imputation model. Keep every available regional DTP3/measles value; fill only unavailable values with a valid national WUENIC percentage for the **same survey year**. This timing matches those survey-time regional measures. Hib3, PCV and rotavirus already use national WUENIC values at **band-entry year**; fill any missing value only if an actual estimate for that exact country-year is available in the declared UNICEF snapshot. Preserve genuine reported zeros. The code does not interpret an absent series or a first-observed year as proof of non-introduction: unverified earlier/no-series zeros remain missing. Verified introduction histories would require a separately documented source rule.

Save the value before substitution, an imputation indicator, national source year and source category for every vaccine. Substitutions remain national values assigned to survey-regions; they do not reconstruct within-country differences. Imputation flags describe provenance and do not automatically add predictor terms. Uncertainty in national estimates or regional departures from them is not propagated by this single substitution. Include an observed-regional-only/reduced-adjustment sensitivity to assess dependence on the fallback. Other selected covariates still require observed usable summaries after the existing HIV imputation.

The [regional pipeline](../R_cbh/covariates/README.md) separately retrieves public indicator snapshots, extracts weighted recode summaries, resolves published regional joins and audits availability. Exposure/region identity must still be resolved; a missing regional vaccination covariate may use the declared UNICEF fallback, without imputing location or PfPR. The [missingness audit](../results/cbh/regional_adjustment_unicef_v2/README.md) distinguishes missing records, entirely lost survey-regions and partially reduced survey-regions. A regional mean can be observed when some individual responses are missing; respondent-level missingness and small denominators are reported separately. Do not drop a mortality record solely because that child's own confounder response is missing.
...
**Availability after UNICEF substitution, 17 September:** compared with the previous 1,015 survey-regions, the revised candidate sample retains **419** and loses **596 (58.7%)**; 217 retained regions also lose some previously included records. Starting from all MAP-eligible data, it contains **1,784,353 child-band records, 598,386 children and 17,822 deaths across 41 surveys in 24 countries**. Before UNICEF substitution it retained 418 regions and 1,783,426 records: the fallback recovers **one additional region and 927 fully eligible records**, because many records with missing regional vaccines also lack other covariates. Missing DTP3 and measles each fall from 3.53% to 0% in the previous fitted sample. Hib3/PCV/rotavirus gaps remain because those inputs already use the same national UNICEF source. See the [updated audit](../results/cbh/regional_adjustment_unicef_v2/README.md); the [earlier audit](../results/cbh/regional_adjustment_v1/README.md) is preserved. These are data-availability counts, not fitted model results.
...
| `data/derived_cbh/regional_adjustment/unicef_v2/regional_covariates_wide.csv` | Current regional overlay after UNICEF fallback, including original values and imputation flags |
| `data/derived_cbh/regional_adjustment/unicef/country_year_estimates.csv` | Actual WUENIC estimates for all five vaccines and their source metadata; no assumed zeros |
| `results/cbh/regional_adjustment_unicef_v2/complete_case_summary.csv` | Current availability after substitution, including losses relative to the previous fitted sample |
