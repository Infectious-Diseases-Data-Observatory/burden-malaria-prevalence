...
nc_one <- function(oc) { f <- fit_c2_primary(d[is.finite(d[[oc]]) & d[[oc]] > 0, ], oc)  # primary M1 spec
  b <- f$beta; s <- f$se
  data.frame(outcome = nc_lab[[oc]], pct = (exp(b)-1)*100,
             lo = (exp(b-1.96*s)-1)*100, hi = (exp(b+1.96*s)-1)*100, p = 2*pnorm(-abs(b/s))) }
nc <- do.call(rbind, lapply(names(nc_lab), nc_one))
...
cat(sprintf("Negative control (primary linear nb-GAM, PfPR2-10>=1%%) — neonatal %+.1f%% per +10 pts (p=%.2f) vs post-neonatal %+.1f%%\n",
            nc$pct[grepl("Neonatal", nc$outcome)], nc$p[grepl("Neonatal", nc$outcome)],
            nc$pct[grepl("post-neonatal", nc$outcome)]))
