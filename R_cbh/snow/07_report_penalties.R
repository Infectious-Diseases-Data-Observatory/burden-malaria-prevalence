...
stopifnot(nrow(diag)==21,nrow(manifest)==21,all(diag$converged),all(diag$finite_covariance),
  all(diag$input_verified),all(diag$rank==diag$coefficients))
ages <- cbh_config()$age_bands$age_band
for(a in ages) {
  z <- diag[diag$age_band==a,]
  stopifnot(nrow(z)==3,length(unique(z$rows))==1,length(unique(z$deaths))==1)
}
...
colors <- setNames(c("#59636B","#CA681E","#207E76"),labels)
types <- setNames(c("solid","dashed","dotdash"),labels)
...
plots <- list(
  all_penalty_splines=plot_curves(central,"Snow prevalence: spline penalisation sensitivities","Two separate changes: higher gamma, or a PfPR shrinkage cubic basis"),
...
md <- c("# Snow-only spline penalisation sensitivities","",
  sprintf("Both analyses use the same %s child-band records and %s deaths from 69 surveys in 31 countries as the full Snow analysis. Surveys and individual interviews after 2015 remain excluded. No MAP estimates or MAP-matched sample restriction enter these fits.",fmt(sum(baseline$rows)),fmt(sum(baseline$deaths))),"",
  "| Series | PfPR basis | Gamma | Calendar-year basis |",
  "|---|---|---:|---|","| Current reference | cr, k=5 | 1 | cr, k=6 |",
  "| Higher gamma | cr, k=5 | 1.4 | cr, k=6 |","| PfPR shrinkage | cs, k=5 | 1 | cr, k=6 |","",
  ...
  vapply(ages,function(a)paste0("| ",a," | ",edf_cell("reference",a)," | ",edf_cell("gamma14",a)," | ",edf_cell("cs",a)," |"),""),"",
  ...
  "| Completed months | Reference | Gamma 1.4 | cs |","|---|---:|---:|---:|",
  vapply(ages,function(a)paste0("| ",a," | ",cell("reference",a)," | ",cell("gamma14",a)," | ",cell("cs",a)," |"),""),"",
