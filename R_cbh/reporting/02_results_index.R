...
writeLines(c("# Key results — primary MAP analysis","",
  ...
  if(mics) c(
    paste("| Supplementary: subgroup refits |",sensitivity("Report","results/cbh/subgroups_dhsmics_map_gamma2_v2/REPORT.md"),"|"),
    paste("| Supplementary: refit without nutrition covariates |",sensitivity("Contrasts","results/cbh/nutrition_adjustment_dhsmics_map_gamma2_v2/CONTRASTS.md"),"|"),
    paste("| Supplementary: imputed covariates |",sensitivity("Report","results/cbh/primary_map_regional17_dhsmics_imputed_gamma2_v6/REPORT.md"),"|")),
  ...
  paste0("Sensitivity analyses are [supplementary](<../Supplementary results/README.md>). The comparator primary remains in `",st$reference,"/`, with a comparison in the new report",
    if(mics) "; the DHS part of this sample reproduces it exactly." else "."),"",
  "## Historical figures","",
