...
sources <- c(model = "New method", ihme_malaria = "IHME", who_cacode = "WHO/UNICEF (CA-CODE)")
...
p <- ggplot(long, aes(year, rate_per100000, colour = source, linetype = source)) +
  ...
  scale_colour_manual(values = c("New method" = "#16747C", "IHME" = "#253746",
    "WHO/UNICEF (CA-CODE)" = "#CC6A30")) +
  scale_linetype_manual(values = c("New method" = "solid", "IHME" = "longdash",
    "WHO/UNICEF (CA-CODE)" = "dotdash")) +
...
caption <- paste(
  ...
  "The new method applies the seven separate primary MAP gamma=2 PfPR effects to annual age-specific IHME all-cause deaths, using the zero-PfPR counterfactual.",
  ...
  "The WHO/UNICEF comparator is the CA-CODE 2026 series from the UN IGME portal, retrieved from UNICEF's CME_CAUSE_OF_DEATH dataflow on 17 September 2026 (under five, both sexes, malaria, deaths); it is not the World Malaria Report all-age series or a fixed proportion of it.",
  ...
  "Model-attributable all-cause mortality reductions and the comparator cause-specific estimates are different estimands. The new method shares IHME all-cause inputs and this comparison is not independent validation.",
...
cbh_atomic_csv(data.frame(file = c(file.path(out, "annual_totals_2004_2024.csv"),
  "R_cbh/reporting/07_annual_mortality_comparison.R"), md5 = vapply(c(file.path(out,
  "annual_totals_2004_2024.csv"), "R_cbh/reporting/07_annual_mortality_comparison.R"), cbh_file_hash, "")),
