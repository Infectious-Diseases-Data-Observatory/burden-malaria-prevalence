#!/usr/bin/env Rscript
# Figure 3 (combined): A = 2024 country comparison with IHME; B = Nigerian
...
## ---- main Figure 3: every panel in deaths per 1,000 under-five child-years -------
# Same three comparisons expressed as mortality rates, so that panels A and B are not
# dominated by population size. In A and in B both axes are linear with the same limits,
# computed per panel from the largest rate plus 5% headroom so the top point is not
# clipped by the frame, and an equality line.
...
rA <- ggplot(cr,aes(ihme_rate,model_rate))+
  geom_abline(slope=1,intercept=0,colour="grey65",linetype=2)+
  geom_point(colour="#215E91",size=2.6,alpha=.85)+
  # Label the large-burden countries, the country with the highest IHME rate, and every
  # country whose two rates differ by at least a factor of 1.65 or by at least 1 death
  # per 1,000 child-years.
  ggrepel::geom_text_repel(data=cr[cr$iso3 %in% c("COD","NGA","AGO","UGA","TZA") |
      cr$ihme_rate==max(cr$ihme_rate) |
...
rB <- ggplot(x,aes(ihme_rate,model_rate,colour=zone))+
  ...
  geom_point(size=2.6)+
  ggrepel::geom_text_repel(aes(label=label),size=3.3,seed=20260917,max.overlaps=Inf,
    min.segment.length=0,segment.colour="grey70",show.legend=FALSE,point.padding=.3,box.padding=.4)+
...
caption <- paste(
  ...
  sprintf("In each of A and B the two axes are linear and share the same limits, set from the largest rate in that panel (A: 0–%s; B: 0–%s deaths per 1,000 child-years). Within each country or state the model and comparator share a denominator, so their ratio is the same as for death counts; the supplementary count version of this figure shows the same comparisons as numbers of deaths.",
    lim_a[2],lim_b[2]),
  definition,caveats)
writeLines(c("# Figure 3 caption","",caption),file.path(out,"CAPTION.md"))
count_caption <- paste(
  paste0("Count version of Figure 3: malaria-attributable deaths before age 5 from the ",model_label," compared with IHME and UN IGME cause-specific malaria death estimates, as numbers of deaths."),
  ...
  definition,caveats)
writeLines(c("# Supplementary figure caption: count version of Figure 3","",count_caption),file.path(out,"CAPTION_counts.md"))
...
message("Combined Figure 3 saved: ",file.path(out,"fig4_burden_comparison.png"))
