...
stopifnot(nrow(manifest)==7L,all(grepl(st$id,manifest$model_file,fixed=TRUE)),
  ...
  nrow(figures)==6L,identical(as.integer(figures$figure),1:6),
...
stopifnot(counts[["primary"]]==sample$records,counts[["deaths"]]==sample$deaths,
  ...
  sum(coverage$regions)==973L,all(coverage$type=="DHS"))
...
writeLines(c("PASS: six main figures reference the revised primary version and match their hashes.",
  "PASS: seven fitted model hashes, convergence flags and gamma=2 verified.",
  "PASS: flow, 100 DHS surveys, 973 survey-regions, child-band/death/child totals reconcile.",
