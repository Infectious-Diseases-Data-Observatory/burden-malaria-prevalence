...
> The commands below default to historical gamma=1 estimates and also prepare inputs reused by gamma=2. Current primary results use the 17-variable regional-adjustment fits in `results/cbh/primary_map_regional17_gamma2_v3/`. Run `Rscript R_cbh/primary/run_regional.R --report-only` for all current figures/tables. The annual (`04_annual_comparison.R`) and state (`05_nigeria_state_burden.R`) entry points now default to that current version; the older commands described below remain historical. See [the audit](../../docs/CODE_AUDIT.md).
...
## Subnational extension: Nigerian states, 2024

`Rscript R_cbh/burden/05_nigeria_state_burden.R` (included in `R_cbh/primary/run.R`) repeats the primary national calculation for each of the 37 Nigerian states, reading the verified primary `pfpr_components.rds`, IHME all-cause deaths and rates by GBD age group and state from `data/external/nigeria_states/`, and state MAP PfPR[2–10] for 2024 from `data/pfpr_admin1_ng_cd_2024.csv`. The age-group mapping, equal 2–4-year split and zero-PfPR contrast are unchanged. The script stops unless every state's bands reproduce its under-5 total and the state bands summed over Nigeria reproduce the national 2024 inputs used by `primary/02_effects.R`. The comparator is IHME's state malaria death rate applied to the all-cause implied person-years. Outputs are in `results/cbh/primary_map_regional17_gamma2_v3/nigeria_states/`: `state_age_estimates_2024.csv`, `state_totals_2024.csv`, `national_reconciliation_2024.csv` (state sum against the national estimate, with the exposure-extraction caveat), `model_specification.txt` and `input_provenance.csv`. The state exposure file is an inherited extraction whose producer is archived (`archive/20_pfpr_admin1_ng_cd.R`); it used MAP admin-1 boundaries and density-only GPW weights, whereas the national series now multiplies density by cell area. Figure 6 and its table come from `R_cbh/reporting/08_nigeria_state_comparison.R`.
