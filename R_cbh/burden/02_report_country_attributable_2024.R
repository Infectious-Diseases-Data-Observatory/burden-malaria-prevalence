...
r <- cbh_read_csv(file.path(out, "country_age_attributable_2024.csv"))
t <- cbh_read_csv(file.path(out, "country_totals_2024.csv"))
...
cbh_atomic_csv(life, file.path(out, "drc_period_life_table_2024.csv"))
...
figure <- ((a | b) / c) + plot_layout(guides = "collect", heights = c(1.15, 1)) +
  plot_annotation(title = "DR Congo: malaria contribution to under-five mortality, 2024",
    ...
    caption = paste("A-B: synthetic cohort under 2024 hazards; point estimates. C: signed rate difference; intervals cover model and HIV-imputation uncertainty only.",
      "Ages 2-4 share one IHME baseline rate; deaths allocated equally. Neonatal: 0-27 days mapped to the <1-month model effect.",
      "MAP 2024 weighted using GPW 2020 population geography. IHME/MAP uncertainty and survey-design uncertainty are not included.", sep = "\n"),
...
ggsave(file.path(out, "drc_malaria_contribution_2024.png"), figure, device = ragg::agg_png,
...
writeLines(c("# National malaria-attributable mortality, 2024", "",
  ...
  "- [DRC period life table](drc_period_life_table_2024.csv), [provenance](provenance.csv)"), file.path(out, "REPORT.md"))
