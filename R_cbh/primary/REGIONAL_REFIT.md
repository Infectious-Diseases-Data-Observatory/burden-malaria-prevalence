...
The current output version is `results/cbh/primary_map_regional17_gamma2_v3/`.
Its [report](../../results/cbh/primary_map_regional17_gamma2_v3/REPORT.md) compares
the seven MAP gamma=2 fits with the preserved 18-variable benchmark in
`results/cbh/primary_map_regional18_gamma2_v2/`.

The 17-variable model uses 13 survey-region summaries and four national annual
covariates, as defined by `cbh_primary_regional_spec()` in `primary/specification.R`.
Sex, multiple births and birth order are removed; maternal age at first birth
replaces age at each birth, and regional wasting/stunting are included.
...
1. `00_prepare_regional.R` assembles existing child-band shards, the audited
   17-variable audited regional overlay (`planned17_audit`) and the fixed HIV incidence panel. It verifies
...
6. The `reporting/` stages produce the inclusion flow, survey map, Figure 3, the combined Figure 4 (`11_burden_comparison_figure.R`: 2024 country comparison, Nigerian states and annual trend as panels A–C), national/annual/state tables and the four-figure paper manifest; `burden/04_annual_comparison.R` and `05_nigeria_state_burden.R` recalculate the associated burden from the saved fits.
...
`data/derived_cbh/models/primary_map_regional17_gamma2_v3/`. Only aggregate
