...
`04_diagnostics.R` exports the aggregate union of included regions per survey. `reporting/03_survey_map.R` combines this with the registry and existing country outlines to regenerate the survey map for the current primary sample (105 surveys/34 countries), without fitting or rebuilding individual records. `reporting/04_paper_figures.R` records the four figure filenames, source hashes and separate captions. These stages are included in the primary runner. The runner writes only inside this project; copying the four PNGs to Overleaf is a separate explicit action and never edits TeX.
