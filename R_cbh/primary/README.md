...
**Current primary (23 September 2026):** the 17-variable regional-adjustment fits on DHS and UNICEF MICS surveys, `primary_map_regional17_dhsmics_gamma2_v5` (132 surveys: 95 DHS and 37 MICS; 36 countries; 7,498,459 child-band records and 102,282 deaths), compared with the DHS-only `primary_map_regional17_gamma2_v3`. Run `Rscript run_all.R --primary` or `Rscript R_cbh/primary/run_regional.R` (default version `regional_mics`; `--dhs-only` reproduces v3). The MICS shards and regional overlay must first be built by `R_mics/` ([MICS pipeline](../../R_mics/README.md)). See [REGIONAL_REFIT.md](REGIONAL_REFIT.md) for prerequisites, stages, version routing, sensitivities and resume/report-only commands. It writes no TeX files.
