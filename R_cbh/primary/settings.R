...
cbh_primary_settings <- function(version=Sys.getenv("CBH_PRIMARY_VERSION","legacy")) {
  stopifnot(version %in% c("legacy","regional","regional18"))
  settings <- list(
  ...
  nthreads = 2L,regional=FALSE,nutrition=FALSE,expected_records=5885022L,expected_deaths=82415L,expected_regions=1015L)
  if(version %in% c("regional","regional18")) {
