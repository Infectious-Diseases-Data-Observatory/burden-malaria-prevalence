#!/usr/bin/env Rscript
# Point-imputation fits for the DHS+MICS imputed-covariate sensitivity (v6): the seven
# primary age-band models (17 covariates, reference knots, gamma = 2, fREML, discrete)
# refitted on the prepared dataset from 03_prepare.R. Same fitting and acceptance rules
# as the other sensitivity refits (R_cbh/sensitivity/nutrition/01_fit.R). Aggregate
# outputs in results/; fitted objects under the ignored data folder.
...
code <- c("R_cbh/sensitivity/imputation_dhsmics/04_fit.R", "R_cbh/sensitivity/imputation_dhsmics/settings.R",
          "R_cbh/sensitivity/model.R", "R_cbh/analysis/model.R", "R_cbh/primary/specification.R")
files <- c(st$data, st$knots, code)
...
input <- prepared$data; input_signature <- prepared$signature; rm(prepared); gc(FALSE)
stopifnot(nrow(input) == st$expected_records, sum(input$death) == st$expected_deaths,
          nlevels(input$region) == st$expected_regions)
form <- cbh_primary_regional_formula(st)
...
run <- function(d, knots, initial = NULL, strict = FALSE) {
  warnings <- character(); set.seed(st$seed)
  ...
  timing <- system.time(f <- withCallingHandlers(bam(form, data = d, knots = knots,
    ...
    gamma = st$gamma, select = FALSE, nthreads = st$nthreads, gc.level = 1,
...
for (i in seq_along(ages)) {
  age <- ages[i]; id <- paste0("imputed_dhsmics_age_", i)
  ...
  knots <- setNames(lapply(c("pfpr_pct", "calendar_year"), function(v) {
    k <- knot_table[knot_table$age_band == age & knot_table$variable == v, ]; k$value[order(k$index)] }), c("pfpr_pct", "calendar_year"))
  stopifnot(length(knots$pfpr_pct) == 5L, length(knots$calendar_year) == 6L,
            min(d$pfpr_pct) >= min(knots$pfpr_pct) - 1e-9, max(d$pfpr_pct) <= max(knots$pfpr_pct) + 1e-9)
  # As in the DHS-only imputed version, 2024 entries (health expenditure now imputed)
  # lie beyond the last calendar-year knot; the cr basis continues linearly there.
  sig <- cbh_hash(list(provenance, st$gamma, st$seed, knots, deparse(form), R.version.string, as.character(packageVersion("mgcv"))))
  ...
  if (is.null(saved) || !identical(saved$signature, sig)) {
    saved <- run(d, knots)
    saved$signature <- sig; saved$input_signature <- input_signature; saved$gamma <- st$gamma; saved$knots <- knots
  ...
  if (!diag$converged || !is.finite(diag$min_smoothing_hessian_eigenvalue) || diag$min_smoothing_hessian_eigenvalue <= 0) {
    ...
    if (is.null(strict) || !identical(strict$signature, sig)) {
      ...
      strict$signature <- sig; strict$input_signature <- input_signature; strict$gamma <- st$gamma; strict$knots <- knots
  ...
  diag$age_band <- age; diag$gamma <- st$gamma; diag$input_verified <- TRUE; diags[[id]] <- diag
  cc <- cbh_sensitivity_curves(f, d, id); cc$series <- "map_full"; curves[[id]] <- cc
  sm <- as.data.frame(summary(f)$s.table); sm$term <- rownames(sm); sm$age_band <- age; smooths[[id]] <- sm
  manifests[[id]] <- data.frame(fit_id = id, age_band = age, model_file = path, md5 = cbh_file_hash(path),
...
message("All seven DHS+MICS imputed-covariate gamma=2 fits completed")
