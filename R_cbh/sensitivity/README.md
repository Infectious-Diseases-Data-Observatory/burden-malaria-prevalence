...
> **Re-pointed at the v7 primary (24 September 2026):** the current primary is `primary_map_regional17_dhsmics_gamma2_v7` (v5 plus Liberia). `subgroups/` writes `results/cbh/subgroups_dhsmics_map_gamma2_v3/`, `nutrition/` writes `results/cbh/nutrition_adjustment_dhsmics_map_gamma2_v3/`, `smc/` writes `results/cbh/smc_dhsmics_map_gamma2_v2/` and [`imputation_dhsmics/`](imputation_dhsmics/README.md) writes `primary_map_regional17_dhsmics_imputed_gamma2_v8`. The v5-based outputs (subgroups and no-nutrition v2, SMC v1, imputed v6) are kept unchanged as history. SMC remains a supplementary result, not part of the paper.

> **SMC before/after (23 September 2026):** [`smc/`](smc/settings.R) adds an admin-1 SMC switch-on indicator to the primary DHS+MICS models in the countries with SMC in `data/SMC_rollout/smc_by_dhs_cluster.csv` (Nigeria by state; main analysis confirmed switch-ons only). `01_assign_smc.R`, `02_fit.R`, `03_report.R`, `04_maps.R`; outputs in `results/cbh/smc_dhsmics_map_gamma2_v2/` (on v7; `smc_dhsmics_map_gamma2_v1/` was fitted on v5).

> **DHS and MICS sample (23 September 2026):** `subgroups/` and `nutrition/` refit the DHS+MICS primary by default (on v5 they wrote `results/cbh/subgroups_dhsmics_map_gamma2_v2/` and `results/cbh/nutrition_adjustment_dhsmics_map_gamma2_v2/`; on v7, v3 as above); pass `--dhs-only` to both the fit and report scripts to reproduce the DHS-only (v3) refits. The imputed-covariate sensitivity on the combined sample is in [`imputation_dhsmics/`](imputation_dhsmics/README.md) (v8 on v7; v6 on v5); `imputation/` remains the DHS-only v4 version.

> **DHS-only subgroup sensitivity (18 September 2026; superseded by the DHS+MICS refits, now `subgroups_dhsmics_map_gamma2_v3`; reproduce with `--dhs-only`):** [`subgroups/`](subgroups/01_fit.R) refits the seven primary 17-variable MAP gamma=2 age-band models separately in four subsets of the fitted sample: Sahel survey regions (centroid ≥12°N, <36°E, outside the Horn), UN M49 Eastern Africa, and surveys up to / after the median survey year. `Rscript R_cbh/sensitivity/subgroups/01_fit.R --dhs-only` then `02_report.R --dhs-only`; outputs in `results/cbh/subgroups_map_gamma2_v1/`, including the supplementary overlay figure `sfig_pfpr_splines_by_subgroup.png`. The older [`sahel/`](sahel/README.md) refit used the 11-variable fit and is superseded. The commands below reproduce historical gamma=1 joint-model comparisons.
...
`imputation/02_supplementary_figure.R` draws the supplementary manuscript figure comparing the complete-case primary and imputed-covariate PfPR curves from the saved comparison curves of `primary_map_regional17_imputed_gamma2_v4`, with paper-facing labels, and writes its caption and provenance (DHS-only history). `R_cbh/reporting/export_paper.py` now copies the DHS+MICS v8 figure and caption written by `imputation_dhsmics/06_report.R` to the manuscript's `Supplementary Figures/` folder instead.
