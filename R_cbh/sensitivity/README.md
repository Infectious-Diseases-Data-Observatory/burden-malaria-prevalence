...
> **SMC before/after (23 September 2026):** [`smc/`](smc/settings.R) adds an admin-1 SMC switch-on indicator to the primary DHS+MICS models in the countries with SMC in `data/SMC_rollout/smc_by_dhs_cluster.csv` (Nigeria by state; main analysis confirmed switch-ons only). `01_assign_smc.R`, `02_fit.R`, `03_report.R`, `04_maps.R`; outputs in `results/cbh/smc_dhsmics_map_gamma2_v1/`.
