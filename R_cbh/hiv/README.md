...
`Rscript R_cbh/hiv/01_fit_incidence.R --extended` adds countries with **no UNAIDS adolescent incidence series** (Liberia, Sao Tome and Principe) with 2000-2024 rows whose adolescent path is a latent AR(1) variable (`a_missing` in `incidence.stan`, `miss_a` index in `hiv_stan_data()`); their child series are then predicted through the same regional and temporal structure with prior-distribution country effects. Outputs are written separately as `child_incidence_country_year_extended.csv`, `child_incidence_draws_extended.rds` and `fit_extended.rds`, with status `imputed_no_adolescent_series`, so the base panel used by the complete-case primaries (v3 and v5; v7 reads it with Liberia added, below) is untouched. The base panel is frozen: without `--extended`, the script refuses to refit it once the model code has changed unless `--force` is given. `--validate-latent` (with `--extended`) treats two observed West and Central Africa countries' adolescent series as latent and withholds their child series, reporting coverage in `latent_adolescent_validation.csv`; `extended_vs_base_panel.csv` compares shared country-years between the two panels.

## Liberia from UNAIDS counts (24 September 2026)

The UNAIDS/UNICEF workbook has no child HIV incidence series for Liberia. `Rscript R_cbh/hiv/03_add_liberia_aidsinfo.R` (after both `01_fit_incidence.R` runs) derives one: rate = 1000 × UNAIDS new HIV infections among children aged 0–14 (AIDSinfo, epidemiological estimates 2026; `data/unaids_aidsinfo/`) / (IHME-implied under-5 person-years − children aged 0–4 living with HIV). Applied to countries with published rates, the same definition reproduces the workbook's child rates (median derived/published 0.988; IQR 0.944–1.055). Liberia's series is 1.32 per 1,000 in 2000, 1.53 in 2005, 1.01 in 2015 and 0.46 in 2024, with status `derived_aidsinfo_counts`; the count bounds are reported but not propagated. The script writes two new panels and leaves the frozen ones unchanged:

- `data/derived_cbh/hiv_incidence/child_incidence_country_year_lbr.csv`: the base panel plus Liberia, every other value unchanged; read by the primary from v7 (`primary_map_regional17_dhsmics_gamma2_v7`).
- `data/derived_cbh/hiv_incidence/child_incidence_country_year_extended_lbr.csv`: the extended panel with Liberia's latent-adolescent imputation replaced by these values; read by the imputed-covariate sensitivity v8, which holds Liberia's value fixed across imputations. São Tomé and Príncipe keeps the latent-adolescent imputation there.

Audit: `results/cbh/hiv_incidence/liberia_aidsinfo/REPORT.md`, with the definition check by country and the comparison with the extended model's imputation.
