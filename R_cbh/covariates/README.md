...
**Current primary policy:** retain usable regional vaccination estimates and replace missing DTP3/measles coverage with exact country/survey-year UNICEF WUENIC estimates. Fill other missing regional summaries with the arithmetic mean of available regions in the same survey, separately by variable; preserve observed values and leave whole-survey gaps unresolved. The current 17-variable availability overlay is `planned17_audit`, with [missingness results](../../results/cbh/planned17_covariate_missingness/README.md). Scripts 11–13 extract maternal age at first birth, fetch WHO-standard regional wasting/stunting and audit the new set. The previous 18-variable overlay/audit `regional_adjustment_reduced_v3` is preserved; scripts 01–10 reproduce that earlier policy. Model preparation uses `R_cbh/primary/specification.R` and the audited new overlay. The 22-variable UNICEF v2 and no-substitution v1 audits remain preserved. Pre-rollout zeros for the three dropped vaccines are irrelevant to the reduced primary selection.
