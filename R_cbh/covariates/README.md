...
**Current primary policy:** retain usable regional vaccination estimates and replace missing DTP3/measles coverage with exact country/survey-year UNICEF WUENIC estimates. Fill other missing regional summaries with the arithmetic mean of available regions in the same survey, separately by variable; preserve observed values and leave whole-survey gaps unresolved. The current overlay/audit version is `regional_adjustment_reduced_v3`. The 22-variable UNICEF v2 and no-substitution v1 audits remain preserved. Pre-rollout zeros for the three dropped vaccines are irrelevant to the reduced primary selection.
