...
`10_prenationwide_zero_scenario.R` evaluates the user-requested scenario of filling only missing Hib3/PCV/rotavirus values strictly before the first reported nationwide introduction year, including partial-rollout years. For countries still reported as not introduced, this extends through the last reported year. It preserves all observed values and leaves rollout-year gaps missing. Outputs under `prenationwide_zero_scenario/` include vaccine-only and all-covariate missingness, affected surveys/countries, and complete-case retention for both the previous-primary and full MAP-eligible samples. The scenario does not change the adopted data policy or overwrite datasets/fits.
